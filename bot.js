...
// 会話履歴を保存するマップ (userId -> messages[])

const conversationHistory = new Map();

const MAX_HISTORY = 10; // 保存する最大メッセージ数 (10往復ではない、メッセージ単体で10個)
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isButton()) {
        ...
        if (serverProcesses[serverType] !== null) {
            ...
            await interaction.reply({ content: '❌ このサーバーは既に起動しています', ephemeral: true });
...
client.on('messageCreate', async (message) => {
    ...
    if (!message.guild) {
        ...
        try {
            ...
            const contextContent = `ユーザー名: ${displayName}\nメッセージ: ${message.content}`;
            ...
            history.push({ role: 'user', content: contextContent });
            ...
            // AIの応答から絵文字を除去 (Unicode Property Escapesを使用)
            ...
            // 次回コンテキストにユーザー名はいらないので、純粋な応答のみ保存
